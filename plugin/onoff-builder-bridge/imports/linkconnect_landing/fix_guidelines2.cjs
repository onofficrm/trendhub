...
if (targetIndex !== -1) {
  content = content.substring(0, targetIndex) + guidelinesSection + '\n      </div>\n    </div>\n  );\n}';
  fs.writeFileSync('src/pages/EventDetail.tsx', content);
  console.log("Guidelines inserted using lastIndexOf");
} else {
  ...
  for(let i=lines.length-1; i>=0; i--) {
     if(lines[i].includes('</ul>')) {
        insertIdx = i + 2; // after </ul> and </div>
        break;
     }
  }
  if(insertIdx !== -1) {
      lines.splice(insertIdx, 0, guidelinesSection);
      fs.writeFileSync('src/pages/EventDetail.tsx', lines.join('\n'));
      console.log("Guidelines inserted using line split");
  }
}
