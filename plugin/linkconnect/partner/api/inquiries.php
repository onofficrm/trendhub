...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if ($iq_id > 0) {
        ...
        if (!$row || (int) $row['pt_id'] !== $pt_id) {
            lc_api_error('문의를 찾을 수 없습니다.', 'NOT_FOUND', 404);
        }
        $rows = array($row);
        $pt_table = lc_table('partners');
        $partner_row = lc_sql_fetch(" SELECT pt_code, pt_name FROM `{$pt_table}` WHERE pt_id = '{$pt_id}' LIMIT 1 ");
        $rows[0]['pt_code'] = $partner_row['pt_code'] ?? '';
        $rows[0]['pt_name'] = $partner_row['pt_name'] ?? '';
        lc_api_success(array(
            'item' => lc_inquiry_to_api($rows[0], true),
        ));
    }

    lc_api_success(array(
        'summary' => lc_inquiry_summary(array('pt_id' => $pt_id)),
        'items'   => array_map(function ($row) {
            return lc_inquiry_to_api($row, false);
        }, lc_inquiry_list(array('pt_id' => $pt_id))),
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $inquiry['pt_code'] = $partner['pt_code'] ?? '';
    $inquiry['pt_name'] = $partner['pt_name'] ?? '';
